...
実装: `kairn/server.py`（mcp 2.x `mcp.server.mcpserver.MCPServer`、streamable HTTP を `/mcp` に提供。UI と同一プロセス）。ツールは 14 個。
...
| `create_case(case, title, workspace?, related?, agent?)` | `case`: 案件 ID（`^[A-Za-z0-9][A-Za-z0-9._-]{0,99}$`）。`title`: 案件名（空・空白のみは拒否）。`related`: 最初から入れる出典（`link_case` と同じ形と検証） | `{case, title, workspace, created: true, related, drive, paths{case_dir, worklog}, note}` | `case.json`（`status: open`、`current_plan: 0`）・`worklog.md` の雛形・`opened` event（`actor: ai`, `agent`）を作る（`store.create_case`）。**作る前に Drive の `cases/<case>/` を `rclone lsf` で 1 回読む**（`sync.drive_case`、10 秒）: `case.json` があれば「別の環境で作られた同じ ID の案件」として拒否（`open_case` で取り寄せて開く）、`case.json` の無いディレクトリがあれば「案件化前のデータ」として拒否（次の `checkin` の rclone sync が `_deleted/` へ退避してしまうため。人が CLI で `checkout` → `kairn new`）。Drive が読めなければ**作る**（`drive.available: false`、`note` に「衝突を確認できていない」）。ローカルに同じ案件があれば拒否（`open_case` を案内）。`related` は形（`validate_related`）と実在（同 ws は自 ws の `cases/`、他 ws は登録済み ws の `cases/`）を検証し、1 つでも不正なら**案件を作らない**。他 ws の案件を入れたときは `xref`（`tool: create_case`）も記録する。**作った案件はローカルだけにある**（Drive に載るのは `checkin`）。ワークスペースは新しい案件 ID から引けないので、登録が複数あれば `workspace` が必須 |
